...
### 4-bit 量化模型 (适用于 Mac MLX)

对于 Mac 用户，我们提供预量化的 4-bit 模型，显著降低内存占用（约 6.6GB）：

| 模型 | 下载方式 |
|------|----------|
| AutoGLM-Phone-9B-4bit | 自行量化（见下方说明） |
...
量化后的模型体积约 6.6GB，可在 16GB 内存的 Mac 上流畅运行。
...
### 4. Mac 本地 MLX 部署 (推荐 Apple Silicon 用户)

对于 Mac 用户，可以使用 Apple MLX 框架实现**完全本地、离线推理**，无需启动服务器：
...
> 💡 详细的 MLX 部署指南请参考 [docs/open_autoglm_guide_cn.md](docs/open_autoglm_guide_cn.md)
